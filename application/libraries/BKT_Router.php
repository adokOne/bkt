...
class Router extends Router_Core {
    ...
    public static $current_rail_version = '2';
    ...
    {
       parent::setup();
       #self::$redirect_url = Input::instance()->xss_clean( self::$redirect_url, 'htmlpurifier' ); 
    ...
    public static function find_uri()
	{
		...
		self::$current_rail_version = Kohana::config('config.default_rail_gds') ? Kohana::config('config.default_rail_gds') : self::$current_rail_version;
        parent::find_uri();
        $lang_prefix = '';
        $cur_domain = '';
        ...
        if(Kohana::config('multi_lang.enabled') and !in_array(self::$current_uri, Kohana::config('ignore.ignore_uri'))){
            ...
            if(preg_match('~^[a-z]{2}(?=/|$)~i', self::$current_uri, $matches) AND isset($matches[0]) OR isset($_SERVER["REDIRECT_URL"]) AND preg_match('(ru)', $_SERVER["REDIRECT_URL"], $matches) AND isset($matches[0])){
        ...
        if(!in_array(self::$current_uri, Kohana::config('ignore.ignore_uri'))){
            ...
            self::$current_uri = self::$current_uri == '' ? $cur_domain .  Kohana::config('routes._default') : $cur_domain  . self::$current_uri;
        }
