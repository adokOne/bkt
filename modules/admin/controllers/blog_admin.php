<?php 
class Blog_Admin extends AdminGrid {
	...
	protected $grid_title = "БЛОГ";
	protected $grid_icon  = "list-alt";
	
    protected $grid_colums = array(
    	"id" => "",
    	"(SELECT title FROM news_langs WHERE news_id = parent.id AND id_lang=1) AS title" => "Тема",
   ...
   /* protected $lang_field = array(
	...
	public function __construct(){
		parent::__construct();
		javascript::add(array('controllers/blog_controller'));
		$this->additional;
	}
    ...
    public function comments($id){
    	javascript::add(array("controllers/blog_com_action_controller"));
    	$comments = ORM::factory('comment')->where("news_lang_id",$id)->find_all();
    	$htm = new View("ad_blog_comment");
    	$htm->comments = $comments;
    	$view = new View('dashboard');
    	$view->modules = ORM::factory('module')->find_all();
    	$view->additional = "";
    	$view->grid_content = $htm->render();
    	
    	$view->render(true);
    }

    public function delete_comment(){
    	$com = ORM::factory('comment')->where("id",$this->input->post("id"))->find();
    	$com->delete();
    ...
    public function get_form(){
    	...
    	$html =  new View('blog_form');
    	$html->data = $data;
    	$html->ne_id = isset($page) ? $page["id"] : 0;
